...
class InheritanceTest < ActiveRecordTestCase
  ...
  test '#69 inheritance' do
    ...
    assert_equal [:born, :conceived], Animal.workflow_spec.states.map(&:name).sort
    assert_equal [:hiding, :upset], Cat.workflow_spec.states.map(&:name).sort, 'Workflow definitions are not inherited'
