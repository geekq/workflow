...
The problems, that I am going to fix:

* replace the extensive usage of method_missing with a simple generation
  of needed functions like `my_state?` and `my_event`.
* do not use ActiveRecord hooks leading to the divergence of `workflow_state`
  table attribute and wokflow.current_state
* fix all the warnings and usage of obsolete API
* fix the API. For example the states() function returns a state object
...
* bidirectional connection between the model
  class and Workflow::Instance - the bind_to, @context, @workflow spagetty.
* The messy and fuzzy API is probably partially caused by RSpec driven
  ...
  orthogonal API. So I am also going to switch to plain old unit tests.
* use update_attribute to immediately save the new state to the database
...
* we've noticed, that mixing the list of events and states with the blocks 
  ...
  due to a deep nesting. We tried and dismissed using lambdas. And desided to
  invoke an optional user defined callback method with the same name as 
  the event (convention over configuration)
  ...
  function `my_transition` (name without exclamation mark). The old way - using
