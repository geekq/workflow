...
Why to overflow the world with yet another fork for the workflow library?
...
* fix the API. For example the states() function returns a state object
  or a string array! While somebody could find the usage like
  `states(states.first)` or bidirectional connection between the model
  class and Workflow::Instance (the bind_to, @context spagetty ) funny,
  it leads to a maintanence nightmare and
  the usage is difficult to explain to the new team members.
* The messy and fuzzy API is probably partially caused by RSpec driven
  ...
  orthogonal API. So I am going to switch to plain old unit tests too.
